...
if __name__ == '__main__':
    ...
    likes = BigList(root="6_hash_small_friendOf", max_length=int(1e07))
    for elem in get_vertical_partitions(key="wsdbm:likes", big_join=False,
                                        int_dict=int_dict, str_dict=str_dict):
        likes.add(elem)

    hasReview = BigList(root="6_hash_small_hasReview", max_length=int(1e07))
    for elem in get_vertical_partitions(key="rev:hasReview", big_join=False,
                                        int_dict=int_dict, str_dict=str_dict):
        hasReview.add(elem)

    Join_2 = BigList(root="6_hash_small_Join_2", max_length=int(1e07))
    for elem in gracehashjoin(likes, hasReview, memory_limit=4, sorted=True):
        Join_2.add(elem)
        if idx % int(1e06) == 0:
            print(idx, elem)
    del likes
    ...
    duration = end - start
    print(f"Time hash grace join sorted small: {duration // 3600}h  {(duration // 60) % 60}min  {duration % 60}sec")
