...
if __name__ == '__main__':
    ...
    follows = BigList(root="2_hash_big_follows", max_length=int(1e07))
    ...
    friendOf = BigList(root="2_hash_big_friendOf", max_length=int(1e07))
    ...
    Join_1 = BigList(root="2_hash_big_Join_1", max_length=int(1e07))
    ...
    likes = BigList(root="2_hash_big_friendOf", max_length=int(1e07))
    for elem in get_vertical_partitions(key="<http://db.uwaterloo.ca/~galuc/wsdbm/likes>", big_join=True,
                                        int_dict=int_dict, str_dict=str_dict):
        likes.add(elem)

    hasReview = BigList(root="2_hash_big_hasReview", max_length=int(1e07))
    for elem in get_vertical_partitions(key="<http://purl.org/stuff/rev#hasReview>", big_join=True,
                                        int_dict=int_dict, str_dict=str_dict):
        hasReview.add(elem)

    Join_2 = BigList(root="2_hash_big_Join_2", max_length=int(1e07))
    for idx, elem in enumerate(gracehashjoin(likes, hasReview, memory_limit=4)):
        Join_2.add(elem)
        if idx % int(1e06) == 0:
            print(idx, elem)
    del likes
